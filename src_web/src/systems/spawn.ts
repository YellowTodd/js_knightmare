...
import type { GameState, ObjectState } from "../types";
...
type SpawnDeps = {
  ...
  spawnBlock: (x: number, y: number, type: number) => void;
  getEnemyLife: (objId: number) => number;
  playerX: number;
  playerY: number;
};

function initializeSpawnedObject(
  object: ObjectState,
  objId: number,
  extra: number,
  getEnemyLife: (objId: number) => number,
...
export function spawnQueuedObject(objId: number, x: number, y: number, extra: number, deps: SpawnDeps): void {
  if (objId <= 0) return;
  if (objId === 22) {
...
export function spawnRowObjects(state: GameState, row: number, deps: SpawnDeps): void {
  if (row < 0) return;
