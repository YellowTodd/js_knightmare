...
type CombatDeps = {
  ...
  playerRect: Rect;
  blockRectSize: { w: number; h: number };
  getShotSprite: (weapon: number) => { x: number; y: number; w: number; h: number };
  getObjectRect: (object: ObjectState) => Rect;
  isSuperWeapon: (weapon: number) => boolean;
  collectBlockBonus: (block: ObjectState) => void;
  hitPlayer: (object: ObjectState) => void;
  hitBlock: (block: ObjectState) => void;
  hitObject: (shot: ShotState | undefined, object: ObjectState) => void;
};
...
function intersectsWithPadding(a: Rect, b: Rect, pad: number): boolean {
  return a.x < b.x + b.w + pad && a.x + a.w > b.x - pad && a.y < b.y + b.h + pad && a.y + a.h > b.y - pad;
}

function shrinkRect(rect: Rect, inset: number): Rect {
  return {
    x: rect.x + inset,
    y: rect.y + inset,
...
export function processCollisions(state: GameState, deps: CombatDeps): void {
  if (deps.controlLocked) return;
  ...
  for (const object of state.objects) {
    ...
    if (!intersects(playerRect, objRect)) continue;
    const prevLives = state.player.lives;
    deps.hitPlayer(object);
    if (!object.active) continue;
    if (state.player.lives < prevLives) return;
  }
  ...
  for (const shot of state.shots) {
    ...
    for (const block of state.blocks) {
      if (!block.active) continue;
      const blockRect = { x: block.x, y: block.y, w: deps.blockRectSize.w, h: deps.blockRectSize.h };
      if (!intersects(shotRect, blockRect)) continue;
      deps.hitBlock(block);
    ...
    for (const object of state.objects) {
      if (!object.active) continue;
      if (object.id === 51) continue;
      if (object.id >= 22 && object.id < 50) continue;
      if (!intersects(shotRect, deps.getObjectRect(object))) continue;
    ...
    for (const enemyShot of state.enemyShots) {
      ...
      if (enemyShot.weapon !== 25 && enemyShot.weapon !== 33) continue;
      ...
      if (!intersects(shotRect, enemyRect)) continue;
