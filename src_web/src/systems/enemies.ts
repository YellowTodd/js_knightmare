...
type EnemyFireDeps = {
  shootChanceByDifficulty: Record<0 | 1 | 2, number[]>;
  random: () => number;
  spawnEnemyShot: (x: number, y: number, vx: number, vy: number, weapon?: number, gpr1?: number, gpr2?: number, gpr3?: number) => void;
};

export function enemiesFire(state: GameState, deps: EnemyFireDeps): void {
  if (state.freezeTimer >= 0) return;
  for (const object of state.objects) {
    if (!object.active || object.id <= 0 || object.id >= 20) continue;
    if (object.y < TILE_SIZE * 2) continue;
    ...
    if (deps.random() > chance) continue;
