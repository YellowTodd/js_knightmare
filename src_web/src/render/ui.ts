import { CANVAS_HEIGHT, CANVAS_WIDTH, GAME_VIEW_HEIGHT, HUD_HEIGHT, HUD_TOP, SCREEN_HEIGHT, SCREEN_OFFSET, SCREEN_WIDTH, TILE_SIZE } from "../config";
import type { GameState } from "../types";
import { drawBitmapText, drawBitmapTextCentered } from "./text";
...
const centerX = (tileCols: number, offsetTiles = 0): number => SCREEN_WIDTH / 2 - (TILE_SIZE * tileCols) / 2 + offsetTiles * TILE_SIZE;
const centerY = (tileRows: number, offsetTiles = 0): number => SCREEN_HEIGHT / 2 - (TILE_SIZE * tileRows) / 2 + offsetTiles * TILE_SIZE;

export function renderHud(ctx: CanvasRenderingContext2D, state: GameState, objectsTileset: HTMLImageElement): void {
  ...
  const placeRight = (endCol: number, value: string): void => {
...
export function renderPhaseOverlay(
  ctx: CanvasRenderingContext2D,
  objectsTileset: HTMLImageElement,
  uiMode: "intro" | "menu" | "stage_ready" | "in_game" | "ending",
  state: GameState
): void {
  if (uiMode !== "in_game") return;
  if (state.phase === "playing") return;
  if (state.phase === "player_dead") return;
...
export function renderUiOverlay(
  ctx: CanvasRenderingContext2D,
  objectsTileset: HTMLImageElement,
  params: {
    uiMode: "intro" | "menu" | "stage_ready" | "in_game" | "ending";
    stage: number;
    bossBlinkGray: boolean;
    introStage: number;
    introLogoY: number;
    menuStartBlinkTick: number;
    menuItem: number;
  ...
  if (params.uiMode === "in_game") return;
  if (params.uiMode === "intro") {
    ...
    ctx.drawImage(
      objectsTileset,
      MENU_CURSOR_SPRITE.x,
      MENU_CURSOR_SPRITE.y,
      MENU_CURSOR_SPRITE.w,
      MENU_CURSOR_SPRITE.h,
      menuTextX + TILE_SIZE,
      menuTextY0 + 1 + params.menuItem * menuLineHeight,
      MENU_CURSOR_SPRITE.w,
      MENU_CURSOR_SPRITE.h
    );
