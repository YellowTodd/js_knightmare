...
function collectBossEyeIndices(state) {
    const indices = [];
...
function spawnBossEyes(state, body, deps) {
...
export function moveBossEyes(state, body) {
    if (state.stage !== 8)
        return;
...
export function spawnBoss(state, deps) {
    if (state.boss.status > 0)
        return;
    ...
    if (deps.uiInGame)
        deps.playBossMusic(state.stage);
}
export function activateBoss(state, deps) {
    if (state.boss.status !== 1)
        return;
    state.boss.status = 2;
...
export function moveBoss(state, deltaSeconds, deps) {
    if (state.boss.status < 2)
        return;
    const body = state.objects[state.boss.bodyIndex];
    const hitbox = state.objects[state.boss.hitboxIndex];
    if (!body || !hitbox || !body.active)
        return;
    state.boss.tick += 1;
    const sprite = deps.getBossSpriteForStage(state.stage);
    ...
    switch (state.stage) {
        ...
        case 5: {
            if (state.boss.tick % 200 === 0) {
                ...
                if (body.x <= TILE_SIZE * 4 && body.gpr2 < 0)
                    body.gpr2 = -body.gpr2;
                if (body.x >= rightEdge && body.gpr2 > 0)
                    body.gpr2 = -body.gpr2;
                body.gpr3 = 0;
                state.boss.shieldActive = true;
            }
        ...
        case 6: {
            if (Math.floor(body.x) === Math.floor(body.gpr2)) {
                if (state.boss.tick % 300 === 0) {
                    ...
                    if (Math.floor(body.x) <= TILE_SIZE * 4)
                        dir = 1;
                    if (Math.floor(body.x) >= rightEdge)
                        dir = -1;
...
export function bossFire(state, deps) {
    if (state.boss.status < 2)
        return;
    const body = state.objects[state.boss.bodyIndex];
    if (!body || !body.active)
...
export function fireBossEyes(state, isBossEyeOpen, spawnEnemyShot) {
    for (const [i, eyeIx] of state.boss.eyeIndices.entries()) {
        const eye = state.objects[eyeIx];
        if (!eye || !eye.active || eye.life <= 0)
            continue;
        if (!isBossEyeOpen(i, eye))
            continue;
...
function isBossShieldBlocking(state) {
    if (!state.boss.shieldActive)
        return false;
    if (state.stage === 3)
        return true;
    if (state.stage === 5)
...
function hitBossEye(state, eye, shot, eyeIndex, deps) {
    const body = state.objects[state.boss.bodyIndex];
    if (!body || !body.active)
        return;
    if (!deps.isBossEyeOpen(eyeIndex, eye))
        return;
...
export function hitBossObject(state, object, shot, deps) {
    if (state.boss.status < 2)
        return;
    const body = state.objects[state.boss.bodyIndex];
    const hitbox = state.objects[state.boss.hitboxIndex];
