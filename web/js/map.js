import { GAME_VIEW_HEIGHT, MAP_COLS, MAP_ROWS, SCREEN_ROWS, SCREEN_WIDTH, TILE_SIZE, TILES_COLS, TILES_OFFSET } from "./config.js";
const mapsTilesetUrl = "./tiles/maps.png";
export async function loadStageMap(stage) {
...
export async function loadMapTileset() {
    const image = new Image();
    image.src = mapsTilesetUrl;
    await image.decode();
    return image;
}
export function isSolidTile(map, row, col) {
...
export function mapCollide(state, px, py) {
    let x = Math.floor(px);
    if (px - x >= 0.9) {
        x += 1;
    }
    ...
    let collide = isSolidTile(state.map, row, col) ||
...
export function renderMap(ctx, state, tileset) {
