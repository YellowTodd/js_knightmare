...
export function getShotSprite(weapon) {
    switch (weapon) {
        case 23:
...
export function getAnimatedShotSprite(shot, timerTick = 0) {
    const animTick = Math.floor(timerTick / 6);
    if (shot.weapon === 25) {
        const phase = Math.abs(Math.floor(shot.gpr1)) % 4;
        if (phase === 3)
            return { ...SHOT_BONE_2_SPRITE, rot: 2 };
        if (phase === 2)
            return SHOT_BONE_3_SPRITE;
        if (phase === 1)
            return SHOT_BONE_2_SPRITE;
        return SHOT_BONE_1_SPRITE;
    }
    if (shot.weapon === 28) {
    ...
    if (shot.weapon === 32) {
        ...
        if (phase < 2)
            return { ...SHOT_AXE_SPRITE, rot: 0 };
        if (phase < 4)
            return { ...SHOT_AXE_SPRITE, rot: 1 };
        if (phase < 6)
            return { ...SHOT_AXE_SPRITE, x: SHOT_AXE_SPRITE.x + TILE_SIZE * 2, rot: 0 };
    ...
    if (shot.weapon === 4 || shot.weapon === 9) {
        ...
        if (phase < 3)
            return { ...SHOT_BOOMERANG_SPRITE, rot: 0 };
        if (phase < 6)
            return { ...SHOT_BOOMERANG_ALT_SPRITE, rot: 0 };
        if (phase < 9)
            return { ...SHOT_BOOMERANG_SPRITE, rot: 3 };
        return { ...SHOT_BOOMERANG_ALT_SPRITE, rot: 3 };
    }
    return getShotSprite(shot.weapon);
}
export function getObjectSprite(id) {
    switch (id) {
        case 1:
            return BLOB_SPRITE;
        case 2:
...
export function getAnimatedObjectSprite(object, timerTick, redDeathPulseTick) {
    ...
    if (object.id >= 1 && object.id < 20) {
        switch (object.id) {
            ...
            case 16:
                ...
                break;
            case 7:
                if (Math.abs(object.gpr3) <= 1 && slowToggle)
                    x += TILE_SIZE * 2;
                break;
            default:
                if (slowToggle)
