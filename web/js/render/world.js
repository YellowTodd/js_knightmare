...
const isQuestionBonus = (object) => (object.id === 20 || object.id === 21) && object.gpr2 <= 2;
const isForegroundPickup = (object) => (object.id === 20 || object.id === 21) && object.gpr2 > 2;
export function renderFinalPortal(ctx, finalPortalActive, finalPortalX, finalPortalY) {
...
export function renderObjects(ctx, objectsTileset, bossTileset, objects, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState) {
    // Layer 1: background terrain effect (id 49).
    // Render first so active entities do not get hidden under it.
    for (const object of objects) {
        if (!object.active || object.id !== 49)
            continue;
        renderObjectSprite(ctx, object, objectsTileset, bossTileset, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState);
    }
    // Layer 2: background bonus crystal forms ("?" group) under active entities.
    for (const object of objects) {
        if (!object.active || !isQuestionBonus(object))
            continue;
        renderObjectSprite(ctx, object, objectsTileset, bossTileset, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState);
    }
    // Layer 3: all regular objects (enemies, boss, effects, etc.).
    for (const object of objects) {
        if (!object.active || object.id === 49 || isQuestionBonus(object) || isForegroundPickup(object))
            continue;
        renderObjectSprite(ctx, object, objectsTileset, bossTileset, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState);
    }
    // Layer 4: foreground pickups (includes visible P power-up forms).
    for (const object of objects) {
        if (!object.active || !isForegroundPickup(object))
            continue;
        renderObjectSprite(ctx, object, objectsTileset, bossTileset, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState);
    }
}
function renderObjectSprite(ctx, object, objectsTileset, bossTileset, stage, bossSpriteOffsetX, getAnimatedObjectSprite, getBossSpriteForStage, getBossRenderAnimState, getBossEyeRenderState) {
    if (object.id === 60)
        return;
    if (object.id === 50) {
        if (object.gpr3 >= 900) {
            ...
            if (eyeAnim.flipX) {
                ...
                ctx.scale(-1, 1);
                ctx.drawImage(bossTileset, BOSS_EYE_SPRITE.x + eyeAnim.xOffset, BOSS_EYE_SPRITE.y + eyeAnim.yOffset, BOSS_EYE_SPRITE.w, BOSS_EYE_SPRITE.h, 0, 0, BOSS_EYE_SPRITE.w, BOSS_EYE_SPRITE.h);
                ctx.restore();
            }
            else {
                ctx.drawImage(bossTileset, BOSS_EYE_SPRITE.x + eyeAnim.xOffset, BOSS_EYE_SPRITE.y + eyeAnim.yOffset, BOSS_EYE_SPRITE.w, BOSS_EYE_SPRITE.h, object.x, object.y, BOSS_EYE_SPRITE.w, BOSS_EYE_SPRITE.h);
            }
            return;
        }
...
export function renderBlocks(ctx, objectsTileset, blocks, getBlockSprite) {
...
export function renderShots(ctx, objectsTileset, shots, getAnimatedShotSprite) {
...
export function renderEnemyShots(ctx, objectsTileset, enemyShots, getAnimatedShotSprite) {
...
function drawShot(ctx, objectsTileset, sprite, x, y) {
...
export function renderShield(ctx, objectsTileset, state) {
    ...
    ctx.drawImage(objectsTileset, shieldX, SHIELD_SPRITE.y, SHIELD_SPRITE.w, SHIELD_SPRITE.h, state.player.x, state.player.y - TILE_SIZE, SHIELD_SPRITE.w, SHIELD_SPRITE.h);
}
export function renderPlayer(ctx, objectsTileset, state, playerAnimCounter) {
    ...
    ctx.drawImage(objectsTileset, walkX + powerOffset, PLAYER_SPRITE.y, PLAYER_SPRITE.w, PLAYER_SPRITE.h, state.player.x, state.player.y, PLAYER_SPRITE.w, PLAYER_SPRITE.h);
}
export function beginGameClip(ctx) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, 0, SCREEN_WIDTH, GAME_VIEW_HEIGHT);
    ctx.clip();
}
export function endGameClip(ctx) {
    ctx.restore();
}
