import { CANVAS_WIDTH, BLOB_SPEED_Y, CANVAS_HEIGHT, GAME_VIEW_HEIGHT, GAME_TICK_MS, HUD_HEIGHT, MAP_COLS, MAP_ROWS_0, PLAYER_INIT_COL, PLAYER_INIT_ROW, PLAYER_INIT_SPEED, PLAYER_MAX_SPEED, PLAYER_SHOT_SPEED_Y, SCREEN_OFFSET, SCREEN_ROWS, SCREEN_WIDTH, SHOT_COOLDOWN_MS, TILE_SIZE } from "./config.js";
import { AudioController } from "./audio.js";
import { MusicController } from "./music.js";
import { CTRL_DOWN, CTRL_FIRE, CTRL_LEFT, CTRL_RESTART, CTRL_RIGHT, CTRL_UP, Input } from "./input.js";
import { loadMapTileset, loadStageMap, mapCollide, renderMap } from "./map.js";
import { renderHud as drawHud, renderPhaseOverlay as drawPhaseOverlay, renderUiOverlay as drawUiOverlay } from "./render/ui.js";
import { beginGameClip, endGameClip, renderBlocks as drawBlocks, renderEnemyShots as drawEnemyShots, renderFinalPortal as drawFinalPortal, renderObjects as drawObjects, renderPlayer as drawPlayer, renderShield as drawShield, renderShots as drawShots } from "./render/world.js";
import { getAnimatedObjectSprite as resolveAnimatedObjectSprite, getAnimatedShotSprite as resolveAnimatedShotSprite, getObjectSprite as resolveObjectSprite, PRINCESS_SPRITE, getShotSprite as resolveShotSprite } from "./render/sprites.js";
import { processCollisions as runCollisionSystem } from "./systems/combat.js";
import { activateBoss as runActivateBoss, bossFire as runBossFire, fireBossEyes as runFireBossEyes, hitBossObject as runHitBossObject, moveBoss as runMoveBoss, moveBossEyes as runMoveBossEyes, spawnBoss as runSpawnBoss } from "./systems/boss.js";
import { enemiesFire as runEnemiesFire } from "./systems/enemies.js";
import { spawnQueuedObject as runSpawnQueuedObject, spawnRowObjects as runSpawnRowObjects } from "./systems/spawn.js";
import { createInitialPlayer } from "./types.js";
import { createIntroState, updateIntroState } from "./ui/intro.js";
import { createMenuState, updateMenuState } from "./ui/menu.js";
...
const BOSS_DIFFICULTY = [0.5, 1, 1.5];
const SHOOT_CHANCE = {
...
export class KnightmareTsPort {
    ...
    uiMode = "intro";
    uiTimer = 180;
    introUi = createIntroState(CANVAS_HEIGHT - 1, SCREEN_ROWS, TILE_SIZE);
    menuUi = createMenuState();
    finalPortalActive = false;
    finalPortalX = SCREEN_WIDTH / 2 - TILE_SIZE;
    finalPortalY = TILE_SIZE;
    controlLocked = false;
    bossSpriteOffsetX = 0;
    bossBlinkGray = false;
    playerAnimCounter = 0;
    playerMovedTick = false;
    redDeathPulseTick = 0;
    constructor(canvas) {
        this.canvas = canvas;
    ...
    updateFinalCutscene(deltaSeconds) {
    ...
    processInput(ctrl, deltaSeconds) {
    ...
    moveObjects(deltaSeconds) {
        for (const object of this.state.objects) {
            if (!object.active)
                continue;
            if (this.state.freezeTimer >= 0 && object.id < 20)
                continue;
            switch (object.id) {
                case 1:
                ...
                case 11:
                    {
                        ...
                        if (object.x < 0)
                            object.x = SCREEN_WIDTH - TILE_SIZE * 2;
                        if (object.x > SCREEN_WIDTH - TILE_SIZE * 2)
                            object.x = 0;
                        object.gpr1 += 90 * deltaSeconds;
                    }
    ...
    hitObject(shot, object) {
        ...
        if (object.id === 20) {
            ...
            if (shot)
                void this.audio.playSfx("power_up_hit");
            return;
        }
        if (object.id === 21) {
        ...
        if (object.life <= 0) {
            ...
            if (object.id < 20)
                void this.audio.playSfx("enemy_kill");
        }
    }
    startFireAnimation(x, y) {
    ...
    makeBridgeGroundFromScreen(x, y) {
        const cell = this.toMapCell(x, y);
        if (!cell)
            return;
        let row = cell.row + 1;
    ...
    toMapCell(x, y) {
        ...
        if (row < 0 || row >= MAP_ROWS_0 || col < 0 || col >= 31)
            return undefined;
        return { row, col };
    }
    makeTileSolid(row, col, solid) {
        if (row < 0 || col < 0 || row * 32 + col >= this.state.map.length)
            return;
        const ix = row * 32 + col;
    ...
    processFreezeTimer(deltaSeconds) {
    ...
    enqueueAction(action, delayTick, repeat = 1, intervalTick = 0, p1 = 0, p2 = 0, p3 = 0, p4 = 0) {
    ...
    processActionQueue() {
        for (const item of this.state.actionQueue) {
            if (!item.active)
                continue;
            if (this.state.timerTick < item.executeAtTick)
                continue;
            switch (item.action) {
                case "spawn_object":
    ...
    spawnQueuedObject(objId, x, y, extra) {
    ...
    setQueuedPhase(phaseCode, timer) {
        if (phaseCode === 1) {
            this.state.phase = "stage_clear";
    ...
    fireBossEyes() {
        runFireBossEyes(this.state, (index, eye) => this.isBossEyeOpen(index, eye), (x, y, vx, vy) => {
    ...
    isSolidAt(row, col) {
        if (row < 0 || row > MAP_ROWS_0 || col < 0 || col >= MAP_COLS)
            return false;
    ...
    tryMovePlayer(dx, dy) {
        ...
        if (this.state.player.x < 0)
            this.state.player.x = SCREEN_WIDTH - TILE_SIZE * 2;
        if (this.state.player.x > SCREEN_WIDTH - TILE_SIZE * 2)
            this.state.player.x = 0;
        if (this.state.player.y < TILE_SIZE * 6)
            this.state.player.y = TILE_SIZE * 6;
        if (this.state.player.y > GAME_VIEW_HEIGHT)
            this.state.player.y = GAME_VIEW_HEIGHT;
        this.checkScrollCollision();
    }
    ...
    renderObjects() {
        drawObjects(this.ctx, this.objectsTileset, this.bossTileset, this.state.objects, this.state.stage, this.bossSpriteOffsetX, (object) => this.getAnimatedObjectSprite(object), this.getBossSpriteForStage.bind(this), this.getBossRenderAnimState.bind(this), this.getBossEyeRenderState.bind(this));
    }
    renderBlocks() {
    ...
    spawnEnemyShot(x, y, vx, vy, weapon = 23, gpr1 = 0, gpr2 = 0, gpr3 = 0) {
    ...
    hasActiveCorePlayerShots() {
        for (let i = 0; i < Math.min(3, this.state.shots.length); i += 1) {
            if (this.state.shots[i]?.active)
                return true;
        }
        return false;
    }
    getShotSprite(weapon) {
        return resolveShotSprite(weapon);
    }
    getAnimatedShotSprite(shot) {
    ...
    getAnimatedObjectSprite(object) {
        return resolveAnimatedObjectSprite(object, this.state.timerTick, this.redDeathPulseTick);
    }
    getBossRenderAnimState(body) {
        if (this.state.boss.status <= 0)
            return { xOffset: 0, yOffset: 0, flipX: false };
        ...
        switch (this.state.stage) {
            case 1:
                if (tick % 8 > 3 && this.state.boss.status !== 1)
                    xOffset += TILE_SIZE * 5;
                break;
            case 5:
                if (tick % 10 > 4 && this.state.boss.status !== 1)
                    xOffset += TILE_SIZE * 5;
                if (body.gpr3 < 0)
                    xOffset += TILE_SIZE * 10;
                break;
            case 6:
                if (tick % 12 > 5 && this.state.boss.status !== 1)
                    xOffset += TILE_SIZE * 5;
                break;
            case 7: {
                const hitbox = this.state.objects[this.state.boss.hitboxIndex];
    ...
    getBossEyeRenderState(index, eye) {
        ...
        if (t < 3 + phaseOffset)
            return { xOffset, yOffset: TILE_SIZE * 2, visible: true, flipX: false };
        ...
        if (t < 112 + phaseOffset)
            return { xOffset, yOffset: TILE_SIZE * 2, visible: true, flipX: false };
        return { xOffset, yOffset: 0, visible: true, flipX: false };
    }
    isBossEyeOpen(index, eye) {
        if (eye.life <= 0)
            return false;
    ...
    getBossSpriteForStage(stage) {
        return BOSS_STAGE_SPRITE[stage] ?? DEFAULT_BOSS_SPRITE;
    }
    getBlockSprite(type, hits, maxHits) {
        if (hits <= 0) {
    ...
    async loadObjectsTileset() {
        const image = new Image();
        image.src = objectsTilesetUrl;
        await image.decode();
        return image;
    }
    async loadBossTileset() {
        const image = new Image();
